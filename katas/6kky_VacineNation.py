...
def vaccine_list(age, status, month):    
    result = []
    vaccines = {'fiveInOne' : ['8 weeks', '12 weeks', '16 weeks'], 'pneumococcal' : ['8 weeks', '16 weeks'], 'rotavirus' : ['8 weeks', '12 weeks'], 
        'meningitisB' : ['8 weeks', '16 weeks', '12 months'], 'hibMenC' : ['12 months'], 'measlesMumpsRubella' : ['12 months', '40 months'], 'preSchoolBooster' : ['40 months']}
    offerFluvacine = ['september', 'october', 'november']

    for k, v in vaccines.items():
        if age in v or status in v:
            result.append(k)        

    if month in offerFluvacine:
        result.append('offer fluVaccine')
    return sorted(result)


#print(vaccine_list('12 weeks','up-to-date','december'))
print(vaccine_list('12 months','16 weeks','june'))
